...
/**
 * Use below code, if you want to print the output to a file
void WriteTour(char *FileName, int *Tour, GainType Cost)
{
    FILE *TourFile;
    int i, j, k, n, Forward, a, b;
    ...
    if (FileName == 0)
    {
        FileName = "default_tour.txt"; // Default filenam
    }
    FullFileName = FullName(FileName, Cost);
    Now = time(&Now);
    if (TraceLevel >= 1)
    ...
    {
        if ((a = Tour[i]) <= n)
            fprintf(TourFile, "%d ", ProblemType != STTSP ? a : NodeSet[a].OriginalId);
        if (Forward)
        {
            if (++i > n)
                i = 1;
        ...
        if (ProblemType == STTSP)
        {
            b = Tour[i];
            for (k = 0; k < NodeSet[a].PathLength[b]; k++)
                fprintf(TourFile, "%d ", NodeSet[a].Path[b][k]);
        }
    }
    fprintf(TourFile, "\n-1\nEOF\n");
    fclose(TourFile);
